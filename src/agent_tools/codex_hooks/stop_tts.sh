...
run_windows_queue() {
  (
    printf '%s' "${message}" | env AGENT_TOOLS_SOURCE="${source_label}" agent-tools ttsify --output-mode play
  ) >>"${child_log}" 2>&1 &
  local child_pid=$!
...
run_file_then_player() {
  ...
  (
    printf '%s' "${message}" | env AGENT_TOOLS_SOURCE="${source_label}" agent-tools ttsify --output-file "${audio_file}"
